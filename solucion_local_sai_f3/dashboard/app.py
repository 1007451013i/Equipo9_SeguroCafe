...
# ---------------------------------------------------------------------------
# MODULO 1. KPIs 6 tarjetas (RMSE, HE, Prima) x Narino / Quindio
# ---------------------------------------------------------------------------
st.markdown(
    """<style>
...
st.header("KPIs del proyecto")

kpi_track_b_raw = _safe(KPIS_ALL, "track_b", default={}) or {}
kpi_act_raw = _safe(KPIS_ALL, "actuariales", default={}) or {}


def _track_b_rows_to_depto_dict(raw_section: dict) -> dict:
    out: dict = {}
    for depto, content in raw_section.items():
        if not isinstance(content, dict):
            continue
        rows = content.get("_rows")
        if isinstance(rows, list):
            by_kpi: dict = {}
            for r in rows:
                if isinstance(r, dict) and "kpi" in r and "valor" in r:
                    by_kpi[str(r["kpi"])] = r["valor"]
            out[depto] = by_kpi
        else:
            out[depto] = {str(k): v for k, v in content.items() if not str(k).startswith("_")}
    return out


kpi_track_b = _track_b_rows_to_depto_dict(kpi_track_b_raw)
kpi_act = kpi_act_raw if isinstance(kpi_act_raw, dict) else {}


def _rmse_of(d: str) -> float | None:
    for k, v in (kpi_track_b.get(d) or {}).items():
        if isinstance(k, str) and "rmse" in k.lower():
            return v
    return None


def _he_of(d: str) -> float | None:
    for k, v in (kpi_track_b.get(d) or {}).items():
        if isinstance(k, str) and k.upper().startswith("HE"):
            return v
    return None


def _prima_of(d: str) -> float | None:
    d_act = kpi_act.get(d)
    if isinstance(d_act, dict):
        for k, v in d_act.items():
            if isinstance(k, str) and "prima" in k.lower():
                return v
    return None


def _riesgo_base_of(d: str) -> float | None:
    d_act = kpi_act.get(d)
    if isinstance(d_act, dict):
        for k, v in d_act.items():
            if isinstance(k, str) and "riesgo" in k.lower() and "base" in k.lower():
                return v
    return None


def _frecuencia_of(d: str) -> float | None:
    d_act = kpi_act.get(d)
    if isinstance(d_act, dict):
        for k, v in d_act.items():
            if isinstance(k, str) and ("frecuencia" in k.lower() or "activacion" in k.lower()):
                return v
    return None


def _kpi_card(col, titulo: str, valor: str, subtitulo: str, delta: str | None = None):
    with col:
        st.metric(titulo, valor, delta=delta, help=subtitulo)
        st.caption(subtitulo)


NAR = "Narino"
QUI = "Quindio"
_cols = st.columns(6, gap="medium")
_fmt_krmse = lambda d: _fmt_num(_rmse_of(d), 1, " kg/ha")
_fmt_khe =   lambda d: _fmt_num(_he_of(d), 3, "")
_fmt_kpri =  lambda d: _fmt_pct(_prima_of(d))

_kpi_card(_cols[0], f"RMSE Holdout - {NAR}", _fmt_krmse(NAR), "Error cuadratico medio holdout; menor = mejor modelo.")
_kpi_card(_cols[1], f"Equidad tarificacion HE - {NAR}", _fmt_khe(NAR), "HE Ederington; mas bajo = mayor equidad.")
_kpi_card(_cols[2], f"Prima actuarial - {NAR}", _fmt_kpri(NAR), "Prima / ha calculada con frecuencia oficial.")
_kpi_card(_cols[3], f"RMSE Holdout - {QUI}", _fmt_krmse(QUI), "Error cuadratico medio holdout; menor = mejor.")
_kpi_card(_cols[4], f"Equidad tarificacion HE - {QUI}", _fmt_khe(QUI), "HE Ederington; mas bajo = mayor equidad.")
_kpi_card(_cols[5], f"Prima actuarial - {QUI}", _fmt_kpri(QUI), "Prima / ha calculada con frecuencia oficial.")

with st.expander("Detalle actuarial / frecuencia activacion"):
    col1, col2 = st.columns(2)
    with col1:
        st.write(f"--- {NAR} ---")
        st.write(f"Riesgo base: {_fmt_pct(_riesgo_base_of(NAR))}")
        st.write(f"Frecuencia activacion: {_fmt_pct(_frecuencia_of(NAR))}")
    with col2:
        st.write(f"--- {QUI} ---")
        st.write(f"Riesgo base: {_fmt_pct(_riesgo_base_of(QUI))}")
        st.write(f"Frecuencia activacion: {_fmt_pct(_frecuencia_of(QUI))}")
        ...
        if resp:
            ...
            try:
                ...
                if v_num < rango_baixo:
                    nivel, color = "BAJO", "#1f77b4"
                elif v_num <= rango_altoo:
                    nivel, color = "MEDIO", "#ff7f0e"
                else:
                    nivel, color = "ALTO", "#2ca02c"
            except Exception:  # noqa: BLE001
            ...
            with col_s:
                ...
                <div style="border:2px solid {color};border-radius:14px;padding:14px 16px;background-color:rgba(255,255,255,0.02);">
                  <div style="font-size:11px;color:#555555;letter-spacing:0.5px;">NIVEL DE RIESGO (vs historico depto)</div>
...
def _reproduccion_en_vivo() -> pd.DataFrame:
    ...
    eventos = [
        (NAR, 2012, "Roya 2012"),
        (NAR, 2015, "El Nino 2015"),
        (QUI, 2012, "Roya 2012"),
        (QUI, 2015, "El Nino 2015"),
    ]
    rows = []
...
# ---------------------------------------------------------------------------
st.markdown("---")
st.header("Calculadora actuarial — Flujo prima vs indemnizaciones")
