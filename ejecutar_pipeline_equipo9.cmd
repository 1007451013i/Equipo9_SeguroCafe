...
REM === 0) Ubicar directorio del CMD y crear logs ===========================
cd /D "%~dp0"
...
REM === 1) Ubicar Python portable (FUERA de bloque SET delay OK) =============
SET "PYEXE="
...
REM === 2) FASE 1/3 — pip install via requirements.txt PINNADAS ===========
ECHO [%%] 5/100 - FASE 1/3 pip install 10 paquetes coherentes... >> _ESTADO.txt
"!PYEXE!" -m pip install --ignore-installed --only-binary=:all: --no-warn-script-location --no-color --progress-bar off -r "!ROOT!requirements.txt" >> logs\_log_pip_std.txt 2>> logs\_log_pip_err.txt
IF %ERRORLEVEL% NEQ 0 (
...
REM === 3) FASE 2/3 — ETL 8 CSVs data/processed ===========================
...
REM === 4) FASE 3/3 — PIPELINE modelos 11 CSVs + 6 PNGs ==================
...
ECHO [%%] 95/100 - FASE 3/3 pipeline OK · 11 CSVs notebooks/outputs + 6 PNGs outputs >> _ESTADO.txt

REM === 5) Conteo final de archivos y status FINAL ========================
SET cntCSVproc=0
SET cntCSVout=0
SET cntPNG=0
...
ECHO CSVs notebooks/outputs:    !cntCSVout!   / 11 minimo >> _ESTADO.txt
ECHO PNGs outputs:              !cntPNG!      / 6  minimo >> _ESTADO.txt

SET "STATUS=INCOMPLETO"
IF !cntCSVproc! GEQ 8 IF !cntCSVout! GEQ 10 IF !cntPNG! GEQ 5 SET "STATUS=COMPLETO 100"
